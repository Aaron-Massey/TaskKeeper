...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (queued_toast)
    toast_API.display(
      queued_toast.message,
      queued_toast.duration,
      queued_toast.icon,
      queued_toast.force_retrigger,
    );
});
...
toast_API.display = function (message, duration, icon, force_retrigger) {
  ...
  if (icon) toast_icon.innerHTML = icons[icon];
  else toast_icon.innerHTML = '';
...
function clipMessage(remainingMessage, num_stages) {
  ...
  const maxSlice = Math.min(
    remainingMessage.length,
    randomIndex + Math.floor(remainingMessage.length / num_stages),
  );
...
function glitchInStep(stages, stage, num_stages, timeout) {
  ...
  if (stages[stage] !== '') toast_text.innerHTML = stages[stage];
