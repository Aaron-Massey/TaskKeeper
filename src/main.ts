...
const renderActions: renderer.TaskRenderActions = {
  onComplete: (component) => completeComponent(component),
...
function statusBarSetDefault() {
  status_bar.reset();
  status_bar.addBlock("new-task", "create", "new task", () =>
    openCreateDialog("task", manager.rootFolder),
  );
  status_bar.addBlock("new-list", "create", "new list", () =>
    openCreateDialog("list", manager.rootFolder),
  );
  status_bar.addBlock(
    "task-count",
    "items",
    manager.rootFolder.getChildren().length.toString(),
  );
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  taskDialog = document.getElementById(
    "task-dialog",
  ...
  taskTitleInput = document.getElementById(
    "task-title-input",
  ) as HTMLInputElement | null;
  taskDescriptionInput = document.getElementById(
    "task-description-input",
  ) as HTMLTextAreaElement | null;
  taskFormCancel = document.getElementById(
    "task-form-cancel",
  ) as HTMLButtonElement | null;
  taskFormSubmit = document.getElementById(
    "task-form-submit",
  ) as HTMLButtonElement | null;
...
function renderTasks(): void {
  ...
  for (const child of children) {
    taskContainer.appendChild(
      renderer.createTask(child, renderActions, manager.rootFolder),
    );
  }
...
function completeComponent(component: TaskComponent): void {
...
function openPromptFallback(
  ...
  const description =
    window.prompt("Description", initialDescription)?.trim() ?? "";
